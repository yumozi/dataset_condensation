...
def main():
    ...
    for exp in range(args.num_exp):
        ...
        for i in range(num_models):
            ...
            trainloader = torch.utils.data.DataLoader(dst_real_train, batch_size=args.batch_train, shuffle=True,
                                                      num_workers=0)
            for il in range(args.inner_loop):
                loss_avg, acc_avg = epoch('train', trainloader, net, optimizer_net, criterion, args,
                      aug=True if args.dsa or args.attack or args.cure else False, adv=True)
                if il % 2 == 0:
                    print(f'Training model {i}, Epoch {il}, loss {loss_avg}, acc {acc_avg}')
            torch.save(net.state_dict(), f'pretrained/mnist/{i}.pth')
        return None"""
        ...
        for it in range(args.Iteration+1):
            ...
            net.train()
            idx = random.randrange(num_models)
            #print(idx)
            net.load_state_dict(torch.load(f'pretrained/mnist/{idx}.pth'))
