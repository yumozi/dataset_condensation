...
def Attack(x, y, net, strategy='', seed = -1, param = None):
    if strategy == 'None' or strategy == 'none' or strategy == '':
        return x
    ...
    if strategy:
        set_seed_Attack(param)
        f = ATTACK_FNS[strategy]
        x = f(x, y, net, param)
        x = x.contiguous()

    return x

def pgd(x, y, net, param):
    attack = torchattacks.PGD(net, eps=param.eps, alpha=param.alpha, steps=param.step)
    x = attack(x, y)
    return x
